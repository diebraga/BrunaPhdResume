import React from "react";

export default function Publications() {
  return (
      <div className="section-content">
        <h1>Pubications</h1>
        <hr/>
        <p className='mt-2'>
        <strong>Nogueira, B. M.</strong>
        (2019) ‘Teaching English as a Second Language to High School students with the support of Facebook and other digital resources’. VI Seminário Web Currículo e XII Encontro de Pesquisadores em Currículo, São Paulo, Brazil, 4-6 Nov. Available: 
        <br/>
        <a style={{overflowWrap: 'break-word', wordWrap: 'break-word'}} href="https://www.academia.edu/42155271/Anais_do_VI_Semin%C3%A1rio_Web_Curr%C3%ADculo_educa%C3%A7%C3%A3o_e_humanismo" target='_blank' rel="noopener noreferrer">https://www.academia.edu/42155271/Anais_do_VI_Semin%C3%A1rio_Web_Curr%C3%ADculo_educa%C3%A7%C3%A3o_e_humanismo</a>
        <br/><br/><br/>
        <strong>Nogueira, B. M.</strong>
        (2016) ‘Education in the contemporary context and the complex thinking’, unpublished dissertation (MSc). Programme of Post-Graduate Studies of Intelligent Technology and Digital Design, Pontifícia Universidade Católica de São Paulo. Available:         <br/>
        <a style={{overflowWrap: 'break-word', wordWrap: 'break-word'}} href="https://tede2.pucsp.br/handle/handle/18962" target='_blank' rel="noopener noreferrer">https://tede2.pucsp.br/handle/handle/18962</a>
        <br/><br/><br/>
        <strong>Nogueira, B. M.</strong>
        (2015) ‘Challenges of the connectivism as a democratic learning theory’. IV Seminário Web Currículo e XII Encontro de Pesquisadores em Currículo, São Paulo, Brazil, 21-23 Sep. Available: 
        <br/>
        <a style={{overflowWrap: 'break-word', wordWrap: 'break-word'}} href="http://www.pucsp.br/webcurriculo/downloads/anais/anais_iv-webcurriculo_2015.pdf" target='_blank' rel="noopener noreferrer">http://www.pucsp.br/webcurriculo/downloads/anais/anais_iv-webcurriculo_2015.pdf</a>
        <br/><br/><br/>
        <strong>Nogueira, B. M., Leite, S. A. S.</strong>
        (2015) ‘Study explores the importance of affectivity in the practices of research supervision’ [press release].  Revista de Educação da PUC Campinas. Available: 
        <br/>
        <a style={{overflowWrap: 'break-word', wordWrap: 'break-word'}} href="http://periodicos.puc-campinas.edu.br/seer/index.php/reveducacao/article/view/2867/1959" target='_blank' rel="noopener noreferrer">http://periodicos.puc-campinas.edu.br/seer/index.php/reveducacao/article/view/2867/1959</a>
        <br/><br/><br/>
        <strong>Nogueira, B. M., Leite, S. A. S.</strong>
        (2014) ‘Affectivity in the process of research supervision’. Revista de Educação da PUC Campinas, v. 19, p. 249-259. Available:        <br/>
        <a style={{overflowWrap: 'break-word', wordWrap: 'break-word'}} href="http://periodicos.puc-campinas.edu.br/seer/index.php/reveducacao/article/view/2852" target='_blank' rel="noopener noreferrer">http://periodicos.puc-campinas.edu.br/seer/index.php/reveducacao/article/view/2852</a>
        <br/><br/><br/>
        <h3>COURSE MATERIAL</h3>
        <strong>Nogueira, B. M.</strong>
        (2019) ‘Understanding connectivism’, EN4022: Educational Technology for Teaching and Learning, University of Limerick, unpublished. Available:
         &nbsp;<a href="">View.</a>
        </p>
      </div>
  );
}
